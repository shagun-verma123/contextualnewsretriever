...
public class SourceNewsStrategy implements NewsRetrievalStrategy {
    ...
    public NewsIntent supportedIntent() {
        return NewsIntent.SOURCE;
    }

    @Override
    public List<ArticleResponse> retrieve(RetrievalCriteria criteria) {
        if (criteria.getSource() == null || criteria.getSource().isBlank()) {
            throw new InvalidRequestException("source is required for the SOURCE intent");
        }
        List<ArticleResponse> articles = repository.findBySourceNameIgnoreCaseOrderByPublicationDateDesc(
                        criteria.getSource(), PageRequest.of(0, criteria.getLimit()))
                .stream()
